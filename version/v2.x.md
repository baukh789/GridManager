...
ts先切换所有文件，对特定的对像暂时使用any
# 改造ts时遇到的问题
- 使用object类型时，无法使用obj[key]方式
开启配置项suppressImplicitAnyIndexErrors

- ts开始前构建大小: 85687，改造后体积变大
1、这是由于ts-loader自已进行了es5转换，原先的babel优化失效了。解决方法：将tsconfig.json中的target调整为es6，并将webpack loader配置中的ts文件解析中将es6转换es5的操作交由bable

2、因为用了ES2017中的特性，所以需要将目标修改为"target": "ES2017", 交给bable进行ES5转化
3、装饰器 在typescript中 每使用一处都会增加体积, 需要确认并找出解决方式

- karma启动报错
需要配置karma中的webapck配置，与主webpack同样的处理方式:增加对ts文件的处理。

- 参数是函数类型时，如何指定参数的类型
待确认是否只能通过interface进行处理

- Settings 应该提供一个类型定义
暂时未处理的原因：Settings需要进行梳理、合并

- jTool需要考虑取消链式操作

- 只有一个export default却提示存在多个export default(TS2528: A module cannot have multiple default exports.)
重新打开文件即可解决

- 部分文件转换后体积增大，如['ajaxPage/index','core/render','core/index','fixed/index']

- 暂未处理的类型[callback, ajaxData, settings, arg]
...
# v2.17.0
    - 新增
        - lineHeight: 行高度，当某个td的高度超出该数值时，当前行的高度将等于该td的高度
        - setLineHeight(table, height): 设置行高度
    - 优化
