...
虚拟滚动待处理
虚拟滚动新增条件验证：每次显示条数大于等于当前数据量 1
数据层新增了lineNum, dom上直接捆绑row。需要验证，如getRowData方法
滚动中的DOM创建过于频繁问题
通栏在切换数据时不能把通栏相关的DOM清理掉
需要验证render中的firstLineKey && lastLineKey有没有不存在的情况，需要在代码中加入详细的备注
...
# v.18.0
    - 新增
        - useVirtualScroll: 启用虚拟滚动，在使用supportTreeData与fullColumn时虚拟滚动无效。
    - 优化
        - 差异化更新DOM
