...
#v2.3.14
- 新增配置参: requestHandler. 请求前处理程序, 可以通过该方法修改全部的请求参数
> 该函数传递参数为 request, 该参数为即将发送的请求参. 更改这个参数后, 接下来的请求参将会使用更改后的参数

- 新增配置参: responseHandler. 执行请求后执行程序, 通过该程序可以修改返回值格式. 仅有成功后该函数才会执行
> 该函数传递参数为 response, 该参数为从远端请求回来的数据. 更改这个参数后, 接下来的表格渲染将会使用更改后的数据
