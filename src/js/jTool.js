...
//            console.log('jTool不支持递归合并');
    ...
    jTool.extend({
        ...
        ,type: function(o){
            ...
            // Element的构造函数会根据不同的元素而产生变化, 所以进行特殊处理
    ...
    *            但是只有click事件可以通过trigger进行调用, 需要修改.(但是通过真实的事件触发,是不会有问题的)
    ...
    * */
    jTool.prototype.extend({
        ...
        ,trigger: function(event){
            jTool.each(this.DOMList, function(index, element){
                try {
                    ...
                    jTool.error('事件:['+ event +']未能正确执行, 请确定方法已经绑定成功');
                }
            });
            return this;
        }
