...
describe('textConfig', () => {
    ...
    it('验证国际化文本总数', () => {
        expect(Object.keys(textConfig).length).toBe(18);
    });
    ...
    it('验证国际化文本[hide-row]初始值', () => {
        expect(textConfig['hide-row']['zh-cn']).toBe('隐藏行');
        expect(textConfig['hide-row']['zh-tw']).toBe('隱藏行');
        expect(textConfig['hide-row']['en-us']).toBe('Hidden Row');
    });
